...
const Datatable = () => {
  ...
  const getrec = async () =>{
    const response = await axios.get("https://paneladminbackend.adaptable.app/Rec/show");
  ...
  const getrecid = async (id)  => {
    const response = await axios.get(`https://paneladminbackend.adaptable.app/Rec/getrec/${userid}`);
    if(response.status===200){
        setData(response.data)
