...
const SingleC = () => {
    ...
    const getSingleClient = async (id)  => {
      const response = await axios.get(`https://paneladminbackend.adaptable.app/Client/searchCl/${id}`);
      if(response.status===200){
     setUser({ ...response.data })
     console.log("data" , response.data)
      }
    ...
    const getSingleAg = async (id)  => {
        const response = await axios.get(`https://paneladminbackend.adaptable.app/agent/searchAg/${id}`);
        if(response.status===200){
       setUser({ ...response.data })
       console.log("data" , response.data)
        }
  ...
  .catch(err =>{
    ...
    const handleSubmite = () => {
        ...
        // Handle validations
        axios
          .put(`https://paneladminbackend.adaptable.app/agent/updatestatuss/${id}`
          ,{ headers: {
            'Content-Type': 'multipart/form-data',
          },})
    ...
    const handleSubmiteCl = () => {
      ...
      // Handle validations
      axios
        .put(`https://paneladminbackend.adaptable.app/Client/updatestatuss/${id}`
        ,{ headers: {
          'Content-Type': 'multipart/form-data',
        },})
